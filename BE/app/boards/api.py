...
# Add a board
@router.post("/add", status_code=status.HTTP_201_CREATED, dependencies = [Depends(require_user)])
def add_a_board(payload: InBoard,
                svc: BoardService = Depends(get_board_service)):
    try:
        board = svc.add_board(payload)
        return {"id": board.id}
    except ValueError as e:
...
# Update a board
@router.put("/update/{board_id}", status_code=status.HTTP_200_OK, dependencies = [Depends(require_user)])
def update_board(board_id: int, payload: InBoard, svc: BoardService = Depends(get_board_service)):
    try:
        board = svc.update_board(board_id, payload)
        return {"id": board.id}
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except Exception:
